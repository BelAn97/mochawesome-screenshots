  ...
  runner.on('end', function () {
    try {
      if (!endCalled) {
        ...
        var passPercentage = obj.stats.testsRegistered - obj.stats.pending > 0 ?
          Math.round((obj.stats.passes / (obj.stats.testsRegistered - obj.stats.pending)) * 1000) / 10 : 0;
        var pendingPercentage = obj.stats.testsRegistered > 0 ?
          Math.round((obj.stats.pending / obj.stats.testsRegistered) * 1000) / 10 : 0;
...
function cleanTest (test) {
  ...
  if (err && err.stack) {
      ...
      cleaned.err = err;
      err.scr = scr;
      err.stack = Highlight.highlightAuto(err.stack).value;
  }
