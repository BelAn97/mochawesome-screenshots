...
class Mochawesome {
  ...
  #setupEventHandlers(runner) {
    ...
    runner.on('end', () => {
      // Wait for async screenshots to complete
      setTimeout(() => this.#handleReportEnd(), 1000);
    });
  }
  ...
  static #buildScreenshotHTML(test) {
    ...
    if (test.customScrFileName && Array.isArray(test.customScrFileName)) {
      return test.customScrFileName
        .filter(entry => entry && entry[0])
        .map(([fileName, msg]) => {
          const msgHtml = msg ? `<p class="scr-msg">${Mochawesome.#escapeHtml(msg)}</p>` : '';
          return `${msgHtml}<img src="./screenshots/${Mochawesome.#escapeHtml(fileName)}">`;
        })
        .join('');
    }
    ...
    // Auto screenshots  
    if (test.scrFileName) {
      const scrHTML = `<img src="./screenshots/${Mochawesome.#escapeHtml(test.scrFileName)}">`;
      return scrHTML;
    }
